...
__all__ = ['PraatTextGrid']


_TextTier = "TextTier"
_IntervalTier = "IntervalTier"

_OOTextFile = re.compile(r"""(?x)
            xmin\ =\ (.*)[\r\n]+
            xmax\ =\ (.*)[\r\n]+
            [\s\S]+?size\ =\ (.*)[\r\n]+ 
""")

_ChronTextFile = re.compile(r"""(?x)
            [\r\n]+(\S+)\ 
            (\S+)\ +!\ Time\ domain.\ *[\r\n]+
            (\S+)\ +!\ Number\ of\ tiers.\ *[\r\n]+"
""")

_OldOOTextFile = re.compile(r"""(?x)
            [\r\n]+(\S+)
            [\r\n]+(\S+)
            [\r\n]+.+[\r\n]+(\S+)
""")
...
class PraatTextGrid(object):
    ...
    def load(file):
        ...
        return PraatTextGrid(open(file).read())
    ...
    def _load_tiers(self, header):
        ...
        if self.text_type == "ChronTextFile":
            ...
            for i in range(0, self.size):
                ...
                tier_info = "\n".join(tier_info)
                tiers.append(PraatTextGridTier(tier_info, self.text_type, self.t_time))
            return tiers
    ...
    def to_oo(self):
        ...
        for i in range(len(self.tiers)):
            ...
            for (x, y) in _curr_tier.header:
                oo_file += "%8s%s = \"%s\"" % ("", x, y)
            if _curr_tier.classid != _TextTier:
...
class PraatTextGridTier(object):
    """ 
...
def _demo_text_grid(demo_data):
    print("** Demo of the TextGrid class. **")

    fid = PraatTextGrid(demo_data)
...
def _demo():
    ...
    print("Format 1")
    _demo_text_grid(_demo_data1)
    print("\nFormat 2")
    _demo_text_grid(_demo_data2)
    print("\nFormat 3")
    _demo_text_grid(_demo_data3)


_demo_data1 = """File type = "ooTextFile"
...
_demo_data2 = """File type = "ooTextFile"
Object class = "TextGrid"
...
_demo_data3 = """"Praat chronological TextGrid text file"
0 2.8   ! Time domain.
2   ! Number of tiers.
