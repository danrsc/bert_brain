...
class CorpusBase:
    ...
    def set_paths_from_path_object(self, path_obj):
        ...
        if self._cache_path is None:
            self._cache_path = os.path.join(
                path_obj.cache_path, type(self).__name__, '{}.npz'.format(self._argument_hash))
    ...
    def load(
        ...
        if paths_obj is not None:
            ...
            self.check_paths()
        result = load_from_cache(self._cache_path, self._bound_arguments, force_cache_miss)
        ...
        save_to_cache(self._cache_path, result, self._bound_arguments)
        return result
