...
class CorpusLoader(object):
    ...
    def load(
        ...
        for corpus in corpora:
            ...
            if type(corpus) in result:
                raise ValueError('Corpus can only be loaded once')

            print('Loading {}...'.format(type(corpus).__name__), end='', flush=True)
            result[type(corpus)] = corpus.load(spacy_tokenizer_model, bert_tokenizer, paths_obj, force_cache_miss)
            print('done')
